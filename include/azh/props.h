...
get_attr_conf_by_preset(const props_attr_preset &preset)
{
...
class props
{
  ...
  public:
    ...
    bool is_load() const
    {
        return !m_xml_path.empty() && std::filesystem::exists(m_xml_path);
    }
    ...
    /* attribute */
    std::string get_attr_by_name(const std::string &name,
                                 const props_attr_preset &preset);
    std::string get_attr_by_name(const std::string &name,
                                 const std::string &condition,
                                 bool isClCompile);
    bool set_attr_by_name(const std::string &name, const std::string &value,
                          const props_attr_preset &preset);
    bool set_attr_by_name(const std::string &name, const std::string &value,
                          const std::string &condition, bool isClCompile);
    bool remove_attr_by_name(const std::string &name,
                             const props_attr_preset &preset);

    bool set_attr(const std::string &name, std::string &value,
                  const props_attr_preset &preset);
    ...
    static props from_project_file(
        const std::string &project_file,
        const std::vector<props_attr_preset> &presets = default_presets)
    {
        ...
        {
            const std::pair<std::string, std::string> conf =
                get_attr_conf_by_preset(preset);
            xpath_strs.push_back("/Project/"
        ...
        {
            pugi::xpath_node source_node =
                source.m_doc.select_node(xpath.c_str());
            ...
            if (source_node.node() && dest_node.node())
            {
                ...
                {
                    pugi::xml_node copyed_node =
                        dest_node.node().append_child(i.name());
  ...
  private:
    void create(const attribute_table_config &debug_conf,

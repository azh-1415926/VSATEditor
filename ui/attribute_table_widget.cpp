...
attribute_table_widget::attribute_table_widget(QWidget *parent)
    : QWidget(parent), ui(new Ui::attribute_table_widget), m_curr_view(attribute_table_view::RAW), m_name("空白属性表")
{
    ui->setupUi(this);
...
void attribute_table_widget::save_as(const QString& file_path,bool to_rename)
{
    ...
    /* 获取属性表完整路径中属性表的文件名，例如 test.props，用作当前属性表编辑窗口的名称 */
    if(to_rename)
    {
        #if (QT_VERSION >= QT_VERSION_CHECK(6,0,0))
            m_name=file_path.last(file_path.size()-file_path.lastIndexOf("/")-1);
            emit rename(this,m_name);
...
void attribute_table_widget::init()
{
    ...
    connect(ui->attrs,&QListWidget::itemClicked,this,[=](QListWidgetItem* item){
        ...
        ui->original_muti_lines_text->setText(std2qstring(value).replace(";","\n"));
    ...
    /* 当单行内容编辑框，内容更改，则将状态切换为 unsave，并更新窗口名称，附带上 * 表明未保存 */
    ...
    /* 当多行内容编辑框，内容更改，则将状态切换为 unsave，并更新窗口名称，附带上 * 表明未保存 */

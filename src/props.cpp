...
void props::save()
{
    ...
    {
        aDebug(AZH_ERROR)
            << "Save - empty xml path, not set default save path.";
        return;
    }

    if (!std::filesystem::exists(
            std::filesystem::path(m_xml_path).parent_path()))
    {
...
bool props::check()
{
    ...
    aDebug(AZH_ERROR)
        << "Check - can not find ClCompile/Link in 'ItemDefinitionGroup'.";
...
std::vector<std::string> props::get_property_sheets()
{
    ...
    for (pugi::xml_node import_group : project.children("ImportGroup"))
    {
        ...
        if (label_attr && std::string(label_attr.value()) == "PropertySheets")
        {
            ...
            {
                pugi::xml_attribute project_attr =
                    import_node.attribute("Project");
                if (project_attr)
                {
...
std::string props::get_attr_by_name(const std::string &name,
                                    const props_attr_preset &preset)
{
    if (!check())
    {
        return "";
    ...
    {
        if (group.attribute("Condition").value() ==
            "'$(Configuration)|$(Platform)'=='" + condition + "'")
        {
...
bool props::set_attr_by_name(const std::string &name, const std::string &value,
                             const props_attr_preset &preset)
{
    if (!check())
    {
...
bool props::set_attr_by_name(const std::string &name, const std::string &value,
                             const std::string &condition, bool isClCompile)
{
    if (!check())
    {
        return false;
...
bool props::remove_attr_by_name(const std::string &name,
                                const props_attr_preset &preset)
{
    if (!check())
    {
...
bool props::set_attr(const std::string &name, std::string &value,
                     const props_attr_preset &preset)
{
    std::pair<std::string, std::string> conf = get_attr_conf_by_preset(preset);
...
bool props::remove_attr(const std::string &name,
                        const props_attr_preset &preset)
{
    std::pair<std::string, std::string> conf = get_attr_conf_by_preset(preset);
...
void props::create(const attribute_table_config &debug_conf,
                   const attribute_table_config &release_conf)
{
    create_root();
...
void props::create_root()
{
    ...
    project.append_attribute("xmlns") =
...
bool props::create_item_group(const attribute_table_config &conf)
{
    ...
    group.append_attribute("Condition") =
        "'$(Configuration)|$(Platform)'=='" + condition + "'";
    ...
    clcompile.append_child("AdditionalOptions")
        .text()
        .set("%(AdditionalOptions) " +
             combine_attribute_value(conf.addl_options, " "));
    clcompile.append_child("PreprocessorDefinitions")
        .text()
        .set("%(PreprocessorDefinitions)" +
             combine_attribute_value(conf.preprocessor_defs));
    // clcompile.append_child("ConformanceMode").text().set("true");
    clcompile.append_child("AdditionalIncludeDirectories")
        .text()
        .set("%(AdditionalIncludeDirectories)" +
             combine_attribute_value(conf.addl_inc_dirs));
    ...
    link.append_child("AdditionalLibraryDirectories")
        .text()
        .set(combine_attribute_value(conf.addl_lib_dirs));
    link.append_child("AdditionalDependencies")
        .text()
        .set("$(CoreLibraryDependencies);%(AdditionalDependencies)" +
             combine_attribute_value(conf.addl_deps));
...
void props::print_content()
{
    ...
    for (pugi::xml_node group : project.children("ItemDefinitionGroup"))
    {
        ...
        if (clCompile)
        {
            ...
            aDebug(AZH_INFO) << "  - WarningLevel: "
                             << clCompile.child("WarningLevel").text().get();
            aDebug(AZH_INFO)
                << "  - SDLCheck: " << clCompile.child("SDLCheck").text().get();
            aDebug(AZH_INFO)
                << "  - PreprocessorDefinitions: "
                << clCompile.child("PreprocessorDefinitions").text().get();
            aDebug(AZH_INFO)
                << "  - AdditionalIncludeDirectories: "
                << clCompile.child("AdditionalIncludeDirectories").text().get();
        }
        ...
        if (link)
        {
            ...
            aDebug(AZH_INFO)
                << "  - SubSystem: " << link.child("SubSystem").text().get();
            aDebug(AZH_INFO)
                << "  - AdditionalLibraryDirectories: "
                << link.child("AdditionalLibraryDirectories").text().get();
            aDebug(AZH_INFO)
                << "  - AdditionalDependencies: "
                << link.child("AdditionalDependencies").text().get();
        }
    }
